...
- Позиции должны быть типовыми для Москвы и сети "__STORE__".
...
Сформируй каталог продуктов для кэширования цен для магазина "__STORE__", город Москва.
...
def build_products_prompt(
    ...
    return (
        PRODUCTS_PROMPT_TEMPLATE.replace("__STORE__", marketplace.value)
        .replace(
            "__CATEGORIES_JSON__",
